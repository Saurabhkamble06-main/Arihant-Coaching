...
export default function RegisterPopup({ onClose, onTriggerLogin }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-[9999]">
      ...
      >
        {/* Close */}
        <button onClick={onClose} className="absolute top-3 right-4 text-gray-600 hover:text-black text-lg">✕</button>

        <h2 className="text-2xl font-extrabold text-center text-blue-700 mb-6">Create Account ✨</h2>

        {error && (
          <div className="bg-red-100 text-red-600 p-2 rounded text-sm mb-4 text-center">{error}</div>
        )}
        ...
        <p className="text-xs text-gray-500 text-center mt-5">
          ...
          <span onClick={() => { onClose(); onTriggerLogin(); }}
